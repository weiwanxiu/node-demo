 ...
 */
function myReadFile(){
	fs.readFile(__dirname + '/test-fs.txt', {flag: 'r+', encoding: 'utf8'}, function (err, data) {
	    if(err) {
	     console.error(err);
	     return;
	    }
...
function myWriteFile(){
	...
	fs.writeFile(__dirname + '/test-fs.txt', w_data, {flag: 'a'}, function (err) {
	   if(err) {
	    console.error(err);
	    } else {
	       console.log('写入成功');
...
function myAppendFile(){
	fs.appendFile(__dirname + '/test-fs.txt', '使用fs.appendFile追加文件内容', function () {
	  console.log('追加内容完成');
	});
	return false;
}
 ...
 */
function myOpen(){
	fs.open(__dirname + '/test-fs.txt', 'r', '0666', function (err, fd) {
	  console.log(fd);
	});
	return false;
}
 ...
 */
function myOpenToCach(){
	fs.open(__dirname + '/test-fs.txt', 'r', function (err, fd) {
	  if(err) {
	    console.error(err);
	    return;
	  } else {
 ...
 */
 function myCachToOpen(){
 	fs.open(__dirname + '/test-fs.txt', 'a', function (err, fd) {
 	  if(err) {
 	    console.error(err);
 	    return;
 	  } else {
 ...
 */
function myUtimes(){
	fs.utimes(__dirname + '/test-fs.txt', new Date(), new Date(), function (err) {
	  if(err) {
	    console.error(err);
	    return;
	  }
	  fs.stat(__dirname + '/test-fs.txt', function (err, stat) {
...
function myWatchFile(){
	fs.watchFile(__dirname + '/test-fs.txt', {interval: 20}, function (curr, prev) {
	...
	fs.watchFile(__dirname + '/test-fs.txt', function (curr, prev) {
 ...
 */
function myUnwatchFile(){
	var listener = function (curr, prev) {
	  console.log('我是监视函数')
	}
	fs.unwatchFile(__dirname + '/test-fs.txt', listener);
	return false;
}
 ...
 */
function myCreateReadStream(){
	var rs = fs.createReadStream(__dirname + '/test-fs.txt', {start: 0, end: 2});
...
function myCreateWriteStream(){
	var ws = fs.createWriteStream(__dirname + '/test-fs.txt', {start: 0});
	...
	ws.end('再见');
	//使用流完成复制文件操作
	var rs = fs.createReadStream(__dirname + '/test-fs.txt')
	var ws = fs.createWriteStream(__dirname + '/test/test-fs.txt');
